...
def get_kota(request, provinsi):
    if request.method == "GET":
        provinsi = Provinsi.objects.get(name=provinsi)
        if provinsi:
...
def get_kecamatan(request, kota):
    if request.method == "GET":
        kota = Kota.objects.get(name=kota)
        if kota:
...
def get_kelurahan(request, kecamatan):
    if request.method == "GET":
        kecamatan = Kecamatan.objects.get(name=kecamatan)
        if kecamatan:
